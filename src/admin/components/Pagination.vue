<template>
  ...
  >
    ...
    <div class="flex items-center gap-x-2">
      ...
      <!-- Prev button -->
      <button class="p-1 rounded hover:bg-gray-200" :disabled="page === 1" @click="previousPage">
      ...
      <!-- Next button -->
      <button class="p-1 rounded hover:bg-gray-200" :disabled="page === lastPage" @click="nextPage">
        <svg class="w-4 h-4 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script setup lang="ts">
  ...
  const emits = defineEmits(['update:perPage', 'update:page', 'change']);

  const listPerPage = [10, 20, 50, 100];
  const perPagePagination = ref(props.perPage);
  const page = ref(props.page);

  // Emit on perPage change
  watch(perPagePagination, value => {
    const perPageInt = Number(value);
